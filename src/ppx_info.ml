  ...
  Exp.record
    [
      ( { txt = Lident "declaration_name"; loc = !default_loc },
        Exp.constant
          (Pconst_string (infos.declaration_name, !default_loc, None)) );
      ...
      ( { txt = Lident "potential_exception"; loc = !default_loc },
        Exp.constant
          (Pconst_string
             (Bool.to_string infos.potential_exception, !default_loc, None)) );
      ( { txt = Lident "potential_catch_exception"; loc = !default_loc },
        Exp.constant
          (Pconst_string
             (Bool.to_string infos.potential_catch_exception, !default_loc, None))
      );
      ( { txt = Lident "declaration_changed"; loc = !default_loc },
        Exp.constant
          (Pconst_string
             (Bool.to_string infos.declaration_changed, !default_loc, None)) );
    ]
    None
